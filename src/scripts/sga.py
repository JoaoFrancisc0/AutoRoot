...
def coleta_mensal(service, driver, selectors, url, folder_id, tipo):
    ...
    ui_actions.preencher_periodo_mensal_passado(driver, selectors["periodo_boleto_fechamento"])
    ui_actions.detectar_e_clicar_n_elementos(driver, selectors["atributos_boleto_fechamento"])
...
def coleta_sga(service, driver, selectors, configs):
    ...
    if (scheduler.verificacao_data_sga(dia, dia_semana, hora)):  
        ...
        if (scheduler.agendamento_boleto_fechamento_mensal(dia, dia_semana, hora)):
            coleta_mensal(service, driver, selectors["supervisao"], url["boleto_url"], folder_id["boleto_fechamento_folder_id"], tipo="boleto_fechamento")

        if (scheduler.agendamento_veiculo_evasao_mensal_e_veiculo_cancelamento_mensal(dia, dia_semana, hora)):
            coleta_mensal(service, driver, selectors["supervisao"], url["veiculo_evasao_url"], folder_id["veiculo_evasao_folder_id"], tipo="veiculo_evasao")
            coleta_mensal(service, driver, selectors["supervisao"], url["veiculo_cancelamento_url"], folder_id["veiculo_cancelamento_folder_id"], tipo="veiculo_cancelamento")
        
        if (scheduler.agendamento_boleto_fechamento_semanal_e_veiculo_cancelamento_semanal(dia, dia_semana, hora)):
            coleta_semanal(service, driver, selectors["supervisao"], url["veiculo_cancelamento_semanal_url"], folder_id["veiculo_cancelamento_semanal_folder_id"], tipo="veiculo_cancelamento_semanal")
